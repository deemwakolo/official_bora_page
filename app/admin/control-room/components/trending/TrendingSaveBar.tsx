...
interface TrendingSaveBarProps {
  status: TrendingSaveStatus;
  message: string | null;
  dirtyCount: number;
...
/*
 * BORA CONTROL ROOM TRENDING SAVE BAR
 *
 * Same interaction language as the Moment Charts and Operations save
 * bars: Clean / Unsaved / Saving / Saved / Error, dirty count,
 * Discard + Save. Saves one platform at a time.
 */
export default function TrendingSaveBar({
  status,
  message,
  dirtyCount,
  platformLabel,
  ...
  const detail =
    message ??
    (status === 'unsaved'
      ? `Local ${platformLabel} draft held — nothing is written until Save.`
      : status === 'saving'
        ? `Committing ${platformLabel} trending...`
        : status === 'saved'
          ? `${platformLabel} trending committed.`
          : status === 'clean'
            ? `No pending ${platformLabel} changes.`
            : 'Nothing was saved.');
